...
constexpr std::chrono::milliseconds _TIME_BETWEEN_TRIES(500);
	...
	constexpr bool _WSAStartup = false;
#endif
...
class Client : public serd_lib::Socket {
	friend class Server;
private:
	std::mutex* mutexSend = nullptr;
	bool in_stop = false;
public:
	Client() = delete;
	Client(const char* address) : Socket(address), mutexSend(new std::mutex) {};
	Client(const char* address, serd_lib::in_port_t port) : Socket(address, port) {};
	Client(const sockaddr_in& address, serd_lib::SocketFD& fd) : Socket(address, fd), mutexSend(new std::mutex) {};
	~Client() {
		in_stop = true;
		mutexSend->lock();
		delete mutexSend;
...
public:
	...
	bool recv(std::string& message, const bool& stop) {
		...
		while (!stop && !in_stop && socketIsValid()) {
			...
			if (ready < 0){
				addError(std::string("poll() failed:") + std::strerror(errno));
				close();
				break;
			};
